...
"""
if old not in s: raise SystemExit('block target missing'); s=s.replace(old,new,1)
old="""                if ($wpdb->query('COMMIT') === false) {
...
"""
if old not in s: raise SystemExit('conversation commit target missing'); s=s.replace(old,new,1)
...
new="""                $wpdb->last_error = '';
                ...
                return self::database_error();
"""
if old not in s: raise SystemExit('race target missing'); s=s.replace(old,new,1)
...
"""
if anchor not in s: raise SystemExit('helper anchor missing'); s=s.replace(anchor,helper+anchor,1)
p.write_text(s)
...
"""
if old not in s: raise SystemExit('unfollow target missing'); p.write_text(s.replace(old,new,1))
...
$check(str_contains($rr,'block_reconciliation_read_failed')&&str_contains($rr,'$own_raw = $wpdb->get_var'),'Yet R2: block/unblock commit reconciliation is DB-error aware.');
$check(str_contains($rr,'direct_conversation_reconciled')&&str_contains($rr,'$ids === $expected'),'Yet R2: direct conversation reconciliation proves exact active membership.');
$check(str_contains($rels,"'follow_database_error','The follow relationship could not be verified.")&&str_contains($rels,"$wpdb->last_error !== ''"),'Yet R2: unfollow cannot treat a failed follow-row read as duplicate inactive success.');
"""
    i=s.rfind('if($fail')
    if i<0: i=s.rfind('if ($fail')
