...
final class SN_Relationships {
    ...
    public static function state(int $viewer_id, int $target_id): array|WP_Error {
        ...
        return [
            ...
            'follow' => [
                'id' => $follow ? (int) $follow->id : 0,
                'state' => $follow_state,
                'version' => $follow ? (int) $follow->version : 0,
            ],
    ...
    public static function follow(int $follower_id, int $followed_id): array|WP_Error {
        ...
        if ($existing && in_array((string) $existing->status, ['active', 'pending'], true)) {
            return self::project($existing, true);
        }
        ...
        if ($ok === false) {
            ...
            if ($race && in_array((string) $race->status, ['active', 'pending'], true)) {
                return self::project($race, true);
            }
        ...
        if (!$row) {
            return new WP_Error('follow_database_error', 'The follow relationship could not be confirmed.', ['status' => 500]);
        }
        SN_DB::add_notification(
            $followed_id,
            $status === 'active' ? 'new_follower' : 'follow_request',
            $status === 'active' ? 'New follower' : 'New follow request',
            '',
            'follow',
            (int) $row->id
        );
    ...
    public static function unfollow(int $follower_id, int $followed_id, int $expected_version = 0): array|WP_Error {
        ...
        if ($expected_version > 0 && (int) $row->version !== $expected_version) {
            return new WP_Error('follow_version_conflict', 'The follow relationship changed before this request was saved.', ['status' => 409]);
        }
        $now = current_time('mysql', true);
    ...
    public static function decide(int $target_id, int $follow_id, string $decision, int $expected_version): array|WP_Error {
        global $wpdb;
        if (!in_array($decision, ['accept', 'reject'], true) || $expected_version <= 0) {
            return new WP_Error('invalid_follow_decision', 'A valid follow decision and version are required.', ['status' => 400]);
        }
        $table = SN_DB::table('follows');
        $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE id=%d", $follow_id));
        if (!$row || (int) $row->followed_id !== $target_id || (string) $row->status !== 'pending') {
            return new WP_Error('follow_request_not_found', 'This follow request is unavailable.', ['status' => 404]);
        }
        if ((int) $row->version !== $expected_version || SN_DB::is_blocked((int) $row->follower_id, $target_id)) {
            return new WP_Error('follow_version_conflict', 'The follow request changed before this decision was saved.', ['status' => 409]);
        }
    ...
    private static function query_list(int $user_id, string $scope, int $limit, string $cursor): array|WP_Error {
        ...
        $table = SN_DB::table('follows');
        if ($scope === 'followers') {
            $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table WHERE followed_id=%d AND status='active' AND id<%d ORDER BY id DESC LIMIT %d", $user_id, $last_id, $limit + 1));
            $other = 'follower_id';
        } elseif ($scope === 'following') {
            $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table WHERE follower_id=%d AND status='active' AND id<%d ORDER BY id DESC LIMIT %d", $user_id, $last_id, $limit + 1));
            $other = 'followed_id';
        } else {
            $rows = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table WHERE followed_id=%d AND status='pending' AND id<%d ORDER BY id DESC LIMIT %d", $user_id, $last_id, $limit + 1));
            $other = 'follower_id';
        }
        if (!is_array($rows)) {
            return new WP_Error('follow_list_error', 'The follow list could not be loaded.', ['status' => 500]);
        }
        ...
        foreach ($rows as $row) {
            ...
            if (!$profile || SN_DB::is_blocked($user_id, (int) $row->{$other})) {
                continue;
            }
    ...
    private static function project(object $row, bool $duplicate = false): array {
        return [
            ...
            'followed_id' => (int) $row->followed_id,
    ...
    private static function decode_cursor(string $cursor, int $user_id, string $scope): int|WP_Error {
        ...
        if (count($parts) !== 2 || !hash_equals(hash_hmac('sha256', $parts[0], wp_salt('nonce')), $parts[1])) {
            return new WP_Error('invalid_follow_cursor', 'The follow cursor is invalid.', ['status' => 400]);
        }
        $body = strtr($parts[0], '-_', '+/');
        $remainder = strlen($body) % 4;
        if ($remainder !== 0) {
            $body .= str_repeat('=', 4 - $remainder);
        }
        $decoded = base64_decode($body, true);
        $data = is_string($decoded) ? json_decode($decoded, true) : null;
        if (!is_array($data) || (int) ($data['user_id'] ?? 0) !== $user_id || (string) ($data['scope'] ?? '') !== $scope || (int) ($data['last_id'] ?? 0) <= 0) {
            return new WP_Error('invalid_follow_cursor', 'The follow cursor is invalid.', ['status' => 400]);
        }
