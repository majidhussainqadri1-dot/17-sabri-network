...
trait SN_Spaces_Part_9 {
    ...
    private static function record(int $space,int $actor,string $action,string $target_type,int $target_id,string $reason,array $scope): void {global $wpdb;$json=wp_json_encode($scope);$scope_hash=hash('sha256',is_string($json)?$json:'');$ok=$wpdb->insert(self::audit_table(),['space_id'=>$space,'actor_id'=>$actor,'action'=>$action,'target_type'=>$target_type,'target_id'=>$target_id,'reason'=>self::text($reason,500),'scope_hash'=>$scope_hash,'created_at'=>self::now()]);if($ok===false)throw new RuntimeException('space_governance_audit_failed');SN_DB::audit($action,'space',$space,'success',['target_type'=>$target_type,'target_id'=>$target_id,'scope_hash'=>$scope_hash],$actor);}
