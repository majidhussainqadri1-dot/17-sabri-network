...
final class SN_Future24_Review_Hardening_B {
    ...
    public static function register(): void {
        ...
        // Run before SN_Future_Superset::cleanup() (default priority 10). Due rows are
        // first claimed as expiry_pending, so the legacy expiry path cannot revoke a
        // membership that has since been promoted or otherwise changed.
    ...
    public static function grant_temporary_membership(WP_REST_Request $r): WP_REST_Response|WP_Error {
        ...
        try {
            ...
            if($member&&(string)$member->status==='active'&&$member->left_at===null){
                ...
                // A changed membership row is no longer owned by this temporary grant.
                if($bound>0&&$bound!==(int)$member->version){
                    self::mark_record_locked($record,'superseded');
                    throw new RuntimeException('permanent_conflict');
                }
            }
            ...
            if($member){
                ...
                if($changed!==1)throw new RuntimeException('member_conflict');
                ...
                if($inserted!==1)throw new RuntimeException('member_write_failed');
                $member_id=(int)$wpdb->insert_id;$new_member_version=1;
            }

            $conversation_id=(int)$space_row->conversation_id;$conversation_member_id=0;
            if($conversation_id>0){
                $cm=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.SN_DB::table('members').' WHERE conversation_id=%d AND user_id=%d LIMIT 1 FOR UPDATE',$conversation_id,$user));
                $conversation_role=$role==='observer'?'member':$role;
            ...
            if($record){
                $cipher=self::encode($record,$payload);if(is_wp_error($cipher))throw new RuntimeException('record_encrypt_failed');
                $changed=$wpdb->update(self::records_table(),['payload_cipher'=>$cipher,'state'=>'active','expires_at'=>$until,'updated_at'=>$at,'version'=>(int)$record->version+1],['id'=>(int)$record->id,'version'=>(int)$record->version]);
                if($changed!==1)throw new RuntimeException('record_conflict');$record_id=(int)$record->id;
            }else{
                $dummy=(object)['feature_id'=>self::TEMP_FEATURE,'owner_id'=>$user,'scope_type'=>'space','scope_id'=>$space];
                $cipher=self::encode($dummy,$payload);if(is_wp_error($cipher))throw new RuntimeException('record_encrypt_failed');
                $client_key=hash('sha256','temporary:'.$space.':'.$user);
                $inserted=$wpdb->insert(self::records_table(),['feature_id'=>self::TEMP_FEATURE,'owner_id'=>$user,'scope_type'=>'space','scope_id'=>$space,'state'=>'active','payload_cipher'=>$cipher,'client_key'=>$client_key,'expires_at'=>$until,'created_at'=>$at,'updated_at'=>$at,'version'=>1]);
                if($inserted!==1)throw new RuntimeException('record_write_failed');$record_id=(int)$wpdb->insert_id;
            }
    ...
    public static function expire_temporary_memberships(): void {
        ...
        // Claim every due active record first. This makes the legacy cleanup path inert
        // even if this queue requires more than one hourly batch to drain.
    ...
    private static function expire_temp_record(int $id): void {
        ...
        try{
            $record=$wpdb->get_row($wpdb->prepare("SELECT * FROM ".self::records_table()." WHERE id=%d AND feature_id=%s AND state='expiry_pending' LIMIT 1 FOR UPDATE",$id,self::TEMP_FEATURE));
            if(!$record){$wpdb->query('ROLLBACK');return;}
            $data=self::decode($record);if(is_wp_error($data))throw new RuntimeException('record_invalid');
            $space=(int)$record->scope_id;$user=(int)($data['user_id']??$record->owner_id);$membership_id=(int)($data['membership_id']??0);$bound_version=(int)($data['membership_version']??0);$at=self::now();
            ...
            $owned=$member&&(string)$member->status==='active'&&$member->left_at===null;
            if($owned&&$bound_version>0)$owned=(int)$member->version===$bound_version;
            if($owned&&$bound_version===0){$owned=(int)$member->approved_by===(int)($data['granted_by']??0)&&(string)$member->role===(string)($data['role']??'');}
            ...
            self::mark_record_locked($record,'expired');
            if($wpdb->query('COMMIT')===false)throw new RuntimeException('commit_failed');
            SN_DB::audit('future_temporary_membership_expired','space',$space,'success',['target_user_id'=>$user,'record_id'=>$id],0);
    ...
    private static function assert_temp_eligibility_locked(int $space,int $user,int $actor): void {
        global $wpdb;$now=self::now();
        if(SN_Policy::is_suspended($user))throw new RuntimeException('blocked');
        ...
        if(!(bool)apply_filters('sn_network_space_temporary_membership_allowed',true,$space,$user,$actor))throw new RuntimeException('policy');
