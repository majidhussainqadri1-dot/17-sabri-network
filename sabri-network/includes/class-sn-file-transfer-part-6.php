...
trait SN_File_Transfer_Part_6 {
    ...
    private static function resolve_recipients(WP_REST_Request $request,int $sender): array|WP_Error {
        global $wpdb;$conversation=absint($request->get_param('conversation_id'));if($conversation){if(!SN_DB::is_member($conversation,$sender))return new WP_Error('conversation_membership_required','An active conversation membership is required.',['status'=>403]);$raw_ids=$wpdb->get_col($wpdb->prepare('SELECT user_id FROM '.SN_DB::table('members').' WHERE conversation_id=%d AND left_at IS NULL AND user_id<>%d',$conversation,$sender));if($wpdb->last_error!==''){SN_DB::audit('file_transfer_recipient_enumeration_failed','conversation',$conversation,'failure',['reason'=>(string)$wpdb->last_error],$sender);return new WP_Error('transfer_recipient_state_unavailable','Conversation recipient state could not be verified safely.',['status'=>503]);}$ids=array_map('intval',is_array($raw_ids)?$raw_ids:[]);}else{$ids=array_values(array_unique(array_filter(array_map('absint',(array)$request->get_param('recipient_ids')))));$ids=array_values(array_diff($ids,[$sender]));}
    ...
    private static function revalidate(object $row,int $user,bool $sender): bool|WP_Error {
        ...
        $recipients=self::recipient_ids((int)$row->id,true);
