...
final class SN_Message_Operations {
    ...
    public static function delete_folder(WP_REST_Request $request): WP_REST_Response|WP_Error {global $wpdb;$id=absint($request['id']);$user=get_current_user_id();$row=self::folder($id,$user);if(!$row)return self::error('sn_folder_missing','The folder is unavailable.',404);try { if ($wpdb->query('START TRANSACTION') === false) throw new RuntimeException('transaction_start_failed');if($wpdb->delete(self::folder_items_table(),['folder_id'=>$id,'user_id'=>$user])===false)throw new RuntimeException('folder_items_delete_failed');if($wpdb->delete(self::folders_table(),['id'=>$id,'user_id'=>$user])!==1)throw new RuntimeException('folder_delete_failed');if($wpdb->query('COMMIT')===false)throw new RuntimeException('folder_delete_commit_failed');return rest_ensure_response(['deleted'=>true]);}catch(Throwable $e){$wpdb->query('ROLLBACK');return self::error('sn_folder_delete_failed','The folder could not be deleted.',500);}}
