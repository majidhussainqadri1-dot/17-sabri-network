...
final class SN_Smail_Runtime_Hardening {
    ...
    public static function save_draft(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        return self::with_locks([$lock],function()use($request,$owner,$public,$wpdb){if($public!==''){$row=$wpdb->get_row($wpdb->prepare('SELECT version FROM '.SN_DB::table('smail_drafts').' WHERE public_id=%s AND owner_id=%d AND deleted_at IS NULL',$public,$owner));if($wpdb->last_error!=='')return new WP_Error('draft_state_unavailable','The Smail draft state could not be read safely.',['status'=>503]);if(!$row)return new WP_Error('draft_not_found','The Smail draft is unavailable.',['status'=>404]);$expected=absint($request->get_param('version'));if($expected<=0)return new WP_Error('draft_version_required','The current draft version is required for updates.',['status'=>400]);if($expected!==(int)$row->version)return new WP_Error('draft_conflict','The Smail draft changed on another device.',['status'=>409]);}return SN_Smail::save_draft($request);});
    }
