<?php
/** Compatibility and corrective route bridges that preserve one canonical File-17 backend. */
...
final class SN_Compatibility_Hardening {
    ...
    public static function secure_forward_message(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        if (!$source || !empty($source->deleted_at) || !SN_DB::is_member((int) $source->conversation_id, $actor) || !SN_DB::is_member($target_id, $actor)) {
        ...
        if ((int) $source->attachment_id > 0 && (string) $source->attachment_source === 'private') {
            return new WP_Error('sn_forward_private_attachment_requires_resend', 'Private attachments must be re-uploaded rather than reused across audiences.', ['status' => 409]);
        }
        ...
        try {
            ...
            if ($wpdb->insert(SN_DB::table('messages'), [
                'conversation_id' => $target_id, 'sender_id' => $actor, 'message_type' => 'text', 'body' => $stored,
                'attachment_id' => 0, 'attachment_source' => 'none', 'reply_to' => 0, 'idempotency_key' => $idem,
                'metadata' => (string) wp_json_encode($metadata), 'created_at' => $now,
            ]) === false) throw new RuntimeException('forward_insert_failed');
            ...
            $event = SN_Outbox::enqueue('message.forwarded', 'message', $new_id, [
                'message_id' => $new_id, 'conversation_id' => $target_id, 'sender_id' => $actor,
                'source_scope_hash' => $source_hash, 'source_visible' => $shared,
