...
final class SN_Compatibility_Hardening {
    ...
    public static function override_privacy_exporter(array $exporters): array {
        if (isset($exporters['sabri-network'])) $exporters['sabri-network']['callback'] = [self::class, 'privacy_export'];
        return $exporters;
    }
    ...
    public static function privacy_export(string $email_address, int $page = 1): array {
        ...
        foreach ($result['data'] as &$item) {
            ...
            foreach ((array) ($item['data'] ?? []) as &$field) if (($field['name'] ?? '') === __('Message', 'sabri-network')) $field['value'] = is_wp_error($plain) ? __('[encrypted message unavailable]', 'sabri-network') : (string) $plain;
            unset($field);
        }
        unset($item);
        return $result;
    ...
    public static function override_routes(): void {
        ...
        register_rest_route('sabri-network/v2', '/messages/(?P<id>\d+)/forward', ['methods' => 'POST', 'callback' => [self::class, 'secure_forward_message'], 'permission_callback' => [SN_REST::class, 'access']], true);
    }
