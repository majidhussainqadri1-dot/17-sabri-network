...
final class SN_Compatibility_Hardening {
    ...
    public static function secure_forward_message(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        if ((string) $target->type === 'direct') {
            ...
            $contact = SN_Policy::can_contact($actor, $peer, 'message'); if (is_wp_error($contact)) return $contact;
        }
    ...
    public static function legacy_heartbeat(WP_REST_Request $request): WP_REST_Response|WP_Error {
        $user_id = get_current_user_id(); $status = sanitize_key((string) $request->get_param('status'));
        ...
        $forward->set_param('device_id', self::legacy_device_id($user_id)); $forward->set_param('state', $status); $forward->set_param('ttl', $status === 'offline' ? 30 : 90); $forward->set_param('label', 'Compatibility web session'); $forward->set_param('capabilities', ['realtime']);
        $response = SN_Presence_Devices::heartbeat($forward); if (is_wp_error($response)) return $response; $data = $response->get_data();
    ...
    public static function legacy_get_presence(WP_REST_Request $request): WP_REST_Response {
        $raw = $request->get_param('user_ids'); if (is_string($raw)) $raw = preg_split('/[^0-9]+/', $raw, -1, PREG_SPLIT_NO_EMPTY);
        $ids = array_slice(array_values(array_unique(array_filter(array_map('absint', (array) $raw)))), 0, 100); $presence = [];
        foreach ($ids as $target_id) {
            $forward = new WP_REST_Request('GET', '/sabri-network/v2/presence/users/' . $target_id); $forward->set_param('user_id', $target_id); $result = SN_Presence_Devices::aggregate($forward);
            if (is_wp_error($result)) continue; $data = $result->get_data(); $presence[] = ['user_id' => (int) ($data['user_id'] ?? $target_id), 'status' => (string) ($data['state'] ?? 'offline'), 'last_seen_at' => $data['last_seen_at'] ?? null];
        }
    ...
    private static function target_audience_is_source_authorized(int $source_id, int $target_id): bool {
        global $wpdb; $targets = array_map('intval', $wpdb->get_col($wpdb->prepare('SELECT user_id FROM ' . SN_DB::table('members') . ' WHERE conversation_id=%d AND left_at IS NULL', $target_id)));
        if (!$targets) return false; foreach ($targets as $user_id) if (!SN_DB::is_member($source_id, $user_id)) return false; return true;
    }

    private static function legacy_device_id(int $user_id): string {
        $session = function_exists('wp_get_session_token') ? (string) wp_get_session_token() : ''; $material = $session !== '' ? $session : ('user:' . $user_id);
