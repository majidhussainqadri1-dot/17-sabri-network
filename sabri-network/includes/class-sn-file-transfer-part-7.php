...
trait SN_File_Transfer_Part_7 {
    private static function format(object $row, int $viewer_id): array { return ['id' => (string) $row->public_id, 'sender_id' => (int) $row->sender_id, 'conversation_id' => (int) $row->conversation_id, 'name' => (string) $row->safe_name, 'mime' => (string) ($row->detected_mime ?: $row->declared_mime), 'size' => (int) $row->total_bytes, 'chunk_size' => (int) $row->chunk_bytes, 'total_chunks' => (int) $row->total_chunks, 'received_chunks' => (int) $row->received_chunks, 'received_bytes' => (int) $row->received_bytes, 'sha256' => (int) $row->sender_id === $viewer_id ? (string) ($row->actual_sha256 ?: $row->expected_sha256) : (string) $row->actual_sha256, 'status' => (string) $row->status, 'scan_status' => (string) $row->scan_status, 'failure_code' => (string) $row->failure_code, 'expires_at' => (string) $row->expires_at, 'created_at' => (string) $row->created_at, 'recipients' => (int) $row->sender_id === $viewer_id ? self::recipient_ids((int) $row->id) : []]; }
    ...
    private static function storage_root(): string {
        $default = trailingslashit(dirname(ABSPATH)) . 'sabri-private/file17-transfers';
        return untrailingslashit((string) apply_filters('sn_network_transfer_storage_root', $default));
    }

    private static function is_safe_storage_root(string $root): bool {
        $normalized = trailingslashit(wp_normalize_path($root));
        $web_root = trailingslashit(wp_normalize_path(ABSPATH));
        if ($normalized === '/' || str_starts_with($normalized, $web_root)) { return false; }
        $resolved = realpath($root);
        $resolved_web = realpath(ABSPATH);
        if ($resolved !== false && $resolved_web !== false && str_starts_with(trailingslashit(wp_normalize_path($resolved)), trailingslashit(wp_normalize_path($resolved_web)))) { return false; }
        return true;
    }
    ...
    public static function render(): string {
        self::register_assets(); wp_enqueue_style('sabri-file-transfer'); wp_enqueue_script('sabri-file-transfer'); $destination = self::url();
        wp_localize_script('sabri-file-transfer', 'SN_TRANSFER_CONFIG', ['restUrl' => esc_url_raw(rest_url('sabri-network/v2/transfers')), 'nonce' => is_user_logged_in() ? wp_create_nonce('wp_rest') : '', 'ready' => self::verified_access() === true, 'maxBytes' => self::MAX_FILE_BYTES, 'defaultChunkBytes' => self::DEFAULT_CHUNK_BYTES, 'loginUrl' => esc_url_raw((string) apply_filters('sn_network_login_url', wp_login_url($destination), $destination))]);
        ob_start(); include SN_DIR . 'templates/file-transfer-app.php'; return (string) ob_get_clean();
    }

    public static function force_content(string $content): string {
        $page_id = (int) get_option('sn_file_transfer_page_id');
        return in_the_loop() && is_main_query() && $page_id > 0 && self::is_owned_page($page_id) && get_queried_object_id() === $page_id ? do_shortcode('[sabri_file_transfer]') : $content;
    }
    ...
    public static function disable_cache(): void {
        ...
        nocache_headers(); header('X-Robots-Tag: noindex, noarchive', true);
    }

    public static function ensure_page(bool $repair): int {
        $id = (int) get_option('sn_file_transfer_page_id'); $page = $id ? get_post($id) : null;
        if ($page instanceof WP_Post && self::is_owned_page($id)) {
            if ($repair || !has_shortcode((string) $page->post_content, 'sabri_file_transfer') || (string) $page->post_status !== 'publish') { wp_update_post(['ID' => $id, 'post_title' => 'File Transfer', 'post_content' => '[sabri_file_transfer]', 'post_status' => 'publish']); }
            return $id;
        }
        $candidate = get_page_by_path('file-transfer', OBJECT, 'page'); if ($candidate instanceof WP_Post && !self::is_owned_page((int) $candidate->ID)) { return 0; }
        $created = $candidate instanceof WP_Post ? (int) $candidate->ID : wp_insert_post(['post_title' => 'File Transfer', 'post_name' => 'file-transfer', 'post_content' => '[sabri_file_transfer]', 'post_status' => 'publish', 'post_type' => 'page', 'comment_status' => 'closed'], true);
        if (is_wp_error($created)) { return 0; }
        $id = (int) $created;
        if ($id > 0) {
            update_post_meta($id, self::PAGE_OWNER_META, 'file-transfer'); update_option('sn_file_transfer_page_id', $id, false);
            if ($candidate instanceof WP_Post && ($repair || !has_shortcode((string) $candidate->post_content, 'sabri_file_transfer') || (string) $candidate->post_status !== 'publish')) { wp_update_post(['ID' => $id, 'post_title' => 'File Transfer', 'post_content' => '[sabri_file_transfer]', 'post_status' => 'publish']); }
        }
        return $id;
    }

    public static function url(): string {
        $id = (int) get_option('sn_file_transfer_page_id');
        if ($id > 0 && self::is_owned_page($id) && get_post_status($id) === 'publish') {
            $url = get_permalink($id);
            if ($url) return (string) $url;
        }
        return home_url('/file-transfer/');
    }

    private static function is_owned_page(int $page_id): bool {
        return $page_id > 0 && (string) get_post_meta($page_id, self::PAGE_OWNER_META, true) === 'file-transfer';
    }
    ...
    public static function export_personal_data(string $email, int $page = 1): array {
        ...
        $rows = $wpdb->get_results($wpdb->prepare('SELECT DISTINCT s.* FROM ' . self::sessions_table() . ' s LEFT JOIN ' . self::recipients_table() . ' r ON r.transfer_id=s.id WHERE s.sender_id=%d OR r.user_id=%d ORDER BY s.id DESC LIMIT 100 OFFSET %d', $user->ID, $user->ID, max(0, ($page - 1) * 100)));
        $data = array_map(static fn($r): array => ['group_id' => 'sabri-file-transfers', 'group_label' => 'Private file transfers', 'item_id' => 'transfer-' . $r->public_id, 'data' => [['name' => 'Identifier', 'value' => $r->public_id], ['name' => 'File name', 'value' => $r->safe_name], ['name' => 'Size', 'value' => $r->total_bytes], ['name' => 'Status', 'value' => $r->status], ['name' => 'SHA-256', 'value' => $r->actual_sha256], ['name' => 'Created', 'value' => $r->created_at], ['name' => 'Expires', 'value' => $r->expires_at]]], $rows ?: []);
