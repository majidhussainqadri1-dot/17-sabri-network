...
trait SN_File_Transfer_Part_7 {
    ...
    public static function disable_cache(): void { if (get_queried_object_id() !== (int) get_option('sn_file_transfer_page_id')) { return; } if (!defined('DONOTCACHEPAGE')) { define('DONOTCACHEPAGE', true); } nocache_headers(); header('X-Robots-Tag: noindex, noarchive', true); }

    public static function ensure_page(bool $repair): int {
        $id = (int) get_option('sn_file_transfer_page_id'); $page = $id ? get_post($id) : null;
        if ($page instanceof WP_Post && (string) get_post_meta($id, self::PAGE_OWNER_META, true) === 'file-transfer') { if ($repair || !has_shortcode((string) $page->post_content, 'sabri_file_transfer')) { wp_update_post(['ID' => $id, 'post_title' => 'File Transfer', 'post_content' => '[sabri_file_transfer]', 'post_status' => 'publish']); } return $id; }
        $candidate = get_page_by_path('file-transfer', OBJECT, 'page'); if ($candidate instanceof WP_Post && (string) get_post_meta((int) $candidate->ID, self::PAGE_OWNER_META, true) !== 'file-transfer') { return 0; }
        ...
        $id = (int) $created;
        if ($id > 0) { update_post_meta($id, self::PAGE_OWNER_META, 'file-transfer'); update_option('sn_file_transfer_page_id', $id, false); } return $id;
    }
