...
trait SN_File_Transfer_Part_8 {
    public static function erase_personal_data(string $email,int $page=1): array {
        ...
        $wpdb->last_error='';$sent_raw=$wpdb->get_col($wpdb->prepare("SELECT id FROM $sessions WHERE sender_id=%d AND status NOT IN ('revoked','expired','rejected') ORDER BY id ASC LIMIT %d",$uid,$page_size));
