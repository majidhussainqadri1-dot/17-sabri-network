...
trait SN_Spaces_Part_6 {
    public static function change_lifecycle(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        if($wpdb->query('START TRANSACTION')===false)return self::error('sn_space_lifecycle_failed','The lifecycle transaction could not start safely.',500);
        try{
    ...
    public static function transfer_owner(WP_REST_Request $request): WP_REST_Response|WP_Error {
        ...
        if($wpdb->query('START TRANSACTION')===false)return self::error('sn_space_transfer_failed','The ownership-transfer transaction could not start safely.',500);
        try{
