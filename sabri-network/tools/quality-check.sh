...
paths=('assets/css/network.css','assets/css/meet.css','assets/css/messages.css','assets/css/message-search.css','assets/css/smail.css','assets/css/file-transfer.css','assets/css/brand-green-overrides.css')
for path in paths:
...
python3 - <<'PY'
from pathlib import Path
main=Path('sabri-network.php').read_text(); smail='\n'.join(Path(p).read_text() for p in ['includes/class-sn-smail.php',*sorted(str(x) for x in Path('includes').glob('class-sn-smail-part-*.php'))]); transfer='\n'.join(Path(p).read_text() for p in ['includes/class-sn-file-transfer.php',*sorted(str(x) for x in Path('includes').glob('class-sn-file-transfer-part-*.php'))]); crypto=Path('includes/class-sn-communication-crypto.php').read_text(); body=Path('includes/class-sn-message-body.php').read_text(); hard=Path('includes/class-sn-central-plan-hardening.php').read_text(); integrity=Path('includes/class-sn-message-integrity.php').read_text(); search=Path('includes/class-sn-message-search.php').read_text()
for marker in ('SN_Smail::register()','SN_File_Transfer::register()',"'file_transfer_max_bytes' => SN_File_Transfer::MAX_FILE_BYTES",'SN_Central_Plan_Hardening::register()'):
...
for marker in ('MAX_FILE_BYTES = 1073741824','x-chunk-sha256','sn_network_transfer_scan_result','Accept-Ranges: bytes','Cache-Control: private, no-store','random_bytes(12)'):
...
assert "'notification_owner' => 'file-19'" in main and 'sn_network_notification_requested' in hard
print('Private communication invariants: PASS')
PY

echo '== Exact staged source manifest and reproducible package =='
BASE='17-sabri-network-and-messages-2.0.2'; rm -rf build; bash tools/package.sh >/tmp/file17-package-first.log
