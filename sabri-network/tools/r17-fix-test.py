...
r17(str_contains($policy,"canonical !== 'adult'"),'Canonical minor/unknown age must be terminal against permissive filters.');
r17(str_contains($policy,"guardian_verified'] !== true) return false"),'Canonical guardian denial must be terminal.');
r17(!str_contains($policy,'sn_account_suspended')&&!str_contains($policy,'sn_date_of_birth')&&!str_contains($policy,'sn_guardian_consent_verified'),'Legacy File-17 metadata must not become an alternate identity authority.');
