...
$check(str_contains($spaces5,"SN_Outbox::enqueue('space.member_unbanned'")&&str_contains($spaces5,'unban_commit_failed'),'Ninth R3: successful unban transitions must enqueue a reliable canonical state-change fact before commit.');
...
$check(substr_count($visibility,'return $id > 0 && !SN_Message_Operations::is_hidden($viewer, $id);')>=2&&!str_contains($visibility,'return $id === 0 || !SN_Message_Operations::is_hidden'),'Ninth R1: malformed message/search projections without a positive canonical ID must fail closed at hidden-message privacy boundaries.');
