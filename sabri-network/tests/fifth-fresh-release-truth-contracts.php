...
$historicalDefects='R1, R2, R3, R4, R5, R6, R7, R8, R9, R10';
...
foreach(['root README'=>$repoReadme,'STATUS'=>$status,'CODING-COMPLETENESS'=>$coding,'plugin readme'=>$pluginReadme,'plugin changelog'=>$pluginChangelog,'candidate boundary'=>$boundary] as $name=>$text){
    $check(str_contains($text,$historicalBranch),"Fresh R10: $name must retain the historical completed review branch attribution.");
    $check(str_contains($text,'60')&&str_contains($text,'10'),"Fresh R10/current: $name must retain historical 60-suite/10-JS release attribution.");
...
$check(str_contains($repoReadme,$historicalDefects)&&str_contains($status,$historicalDefects)&&str_contains($pluginReadme,$historicalDefects),'Fresh R10: historical release/status attribution must preserve that completed-cycle defect-bearing record.');
$check(str_contains($repoReadme,'R5')&&str_contains($status,'R5')&&str_contains($pluginReadme,'R5'),'Fresh R10: historical release/status attribution must retain the completed-cycle R5 record.');
