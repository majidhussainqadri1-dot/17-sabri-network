...
$main=$read($root.'/sabri-network.php');$activator=$read($root.'/includes/class-sn-activator.php');$migration=$read($root.'/includes/class-sn-fifth-fresh-migration-hardening.php');$spaces='';foreach(glob($root.'/includes/class-sn-spaces*.php')?:[] as $space_file)$spaces.=$read($space_file);$risk=$read($root.'/includes/class-sn-high-risk.php');$presence=$read($root.'/includes/class-sn-presence-devices.php');$ops=$read($root.'/includes/class-sn-message-operations.php');$contexts=$read($root.'/includes/class-sn-context-adapters.php');$conference=$read($root.'/includes/class-sn-conference-provider.php');
...
$check(str_contains($migration,"[SN_High_Risk::class,'install']")&&!str_contains($main,'SN_High_Risk::install()'),'High-risk schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($migration,"[SN_Spaces::class,'install']")&&!str_contains($main,'SN_Spaces::install()'),'Spaces schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($migration,"[SN_Presence_Devices::class,'install']")&&!str_contains($main,'SN_Presence_Devices::install()'),'Presence schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($migration,"[SN_Message_Operations::class,'install']")&&!str_contains($main,'SN_Message_Operations::install()'),'Message operations schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($migration,"[SN_Context_Adapters::class,'install']")&&!str_contains($main,'SN_Context_Adapters::install()'),'Context schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($migration,"[SN_Conference_Provider::class,'install']")&&!str_contains($main,'SN_Conference_Provider::install()'),'Conference schema lifecycle must be owned by serialized migration governance.');
$check(str_contains($activator,'SN_Fifth_Fresh_Migration_Hardening::upgrade(true)')&&str_contains($migration,"[SN_High_Risk::class,'install']")&&str_contains($migration,"[SN_Spaces::class,'install']")&&str_contains($migration,"[SN_Presence_Devices::class,'install']")&&str_contains($migration,"[SN_Message_Operations::class,'install']")&&str_contains($migration,"[SN_Context_Adapters::class,'install']")&&str_contains($migration,"[SN_Conference_Provider::class,'install']"),'Fresh activation must delegate every pre-2.0 schema domain to the serialized migration governor before version truth is published.');
$check(str_contains($migration,"[SN_File_Transfer::class,'install']")&&str_contains($migration,"[SN_Smail::class,'install']")&&str_contains($activator,'SN_File_Transfer::ensure_storage()')&&str_contains($activator,'SN_File_Transfer::ensure_page(false)')&&str_contains($activator,'SN_Smail::ensure_page(false)'),'Fresh activation must install through the governor and bind Smail and verified private transfer before activation completes.');
