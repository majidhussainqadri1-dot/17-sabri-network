...
$check(str_contains($m,"'completion_path'=>'verified-pre-lock-state-repair'")&&str_contains($m,"new WP_Error('sn_migration_state_unavailable'")&&str_contains($m,"(\$stored_state['status'] ?? '') !== 'complete'"),'Fresh20 R01: a verified pre-lock fast path must repair and prove durable complete migration-state truth before success.');
$check(str_contains($m,'private static function verify_version_truth()')&&str_contains($m,"'sn_file_transfer_schema_version'=>'1.0.0'")&&str_contains($m,"'sn_two_plan_schema_version'=>'2.1.0'"),'Fresh20 R05: centralized migration success must include installer-local version truth.');
