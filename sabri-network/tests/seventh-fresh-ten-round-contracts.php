...
$check(str_contains($migration,"'completion_path'=>'post-lock-fast-path'")&&str_contains($migration,"'status'=>'complete'")&&str_contains($migration,"if (!\$force && (string)get_option('sn_plugin_version','') === SN_VERSION && self::verify_schema() && self::verify_version_truth()) {"),'Fresh R1: verified post-lock migration fast-path must republish complete migration state before returning.');
