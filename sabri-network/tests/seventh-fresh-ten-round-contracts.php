...
$check(str_contains($callRuntime,"['methods'=>'POST','callback'=>[self::class,'create_meeting']")&&str_contains($callRuntime,'private static function same_meeting_create_request'),'Fresh R6: the final Meet-create route must own exact-request idempotency verification.');
...
$check(str_contains($futureG,'private static function purge_confirmed')&&str_contains($futureG,"['purged','deleted','removed']")&&substr_count($futureG,'self::purge_confirmed($purge)')>=2,'Fresh R8: semantic consent withdrawal and retry must require an explicit terminal purge confirmation.');
$check(!str_contains($futureG,'$purge!==null&&$purge!==false&&!is_wp_error($purge)'),'Fresh R8: ambiguous non-null semantic purge adapter results must never clear purge-pending state.');
$check(str_contains($futureH,'private static function same_bridge_configuration')&&str_contains($futureH,"['subtype','protocol','remote_endpoint','remote_room','credential_ref','direction','identity_authority']"),'Fresh R8: interoperability bridge duplicate resolution must compare all stable material bridge configuration fields.');
$check(str_contains($futureH,"self::error('sn_interop_idempotency_conflict'") ,'Fresh R8: mismatched interoperability bridge reuse must fail with an explicit idempotency conflict.');
$check(str_contains($futureH,'$response_data=$committed')&&str_contains($futureH,'self::public_bridge($response_data)'),'Fresh R8: a true interoperability duplicate must report committed canonical bridge truth, not newly requested unsaved data.');
...
$check(str_contains($readme,'57 PHP review suites')&&str_contains($readme,'10 JavaScript syntax entry points'),'Later R1: readme release truth must match the current explicit QA inventory.');
$check(str_contains($changelog,'57 PHP review suites')&&str_contains($changelog,'10 JavaScript syntax entry points'),'Later R1: changelog release truth must match the current explicit QA inventory.');
...
foreach(['root README'=>$repoReadme,'STATUS'=>$status,'CODING-COMPLETENESS'=>$coding,'QA-INVENTORY'=>$qa,'CURRENT-CANDIDATE-BOUNDARY'=>$boundary] as $name=>$text){
    $check(str_contains($text,'57')&&str_contains($text,'10'),"Later R2: $name must reflect the current 57-suite/10-JS quality truth.");
