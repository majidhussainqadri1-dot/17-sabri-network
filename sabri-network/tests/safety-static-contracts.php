...
$policy = safety_content('includes/class-sn-policy.php');
$auth = safety_content('includes/class-sn-auth.php');
...
safety_check(str_contains($db, "DB_VERSION = '2.0.4'"), 'The database schema version must advance for report appeals and decision evidence.');
...
safety_check(str_contains($db, 'decision_reason TEXT') && str_contains($db, 'decision_by BIGINT UNSIGNED') && str_contains($db, 'decision_at DATETIME'), 'Triage decisions must preserve a reasoned decision record.');
...
safety_check(str_contains($rest, "'/reports/(?P<id>\\d+)/appeal'") && str_contains($rest, 'appeal_report'), 'Reported users must have an authenticated appeal route.');
safety_check(str_contains($rest, "'/admin/reports/(?P<id>\\d+)/appeal'") && str_contains($rest, 'admin_decide_report_appeal'), 'A separate administrator appeal-decision route must exist.');
...
safety_check(str_contains($rest, 'SN_Safety::can_transition_status') && str_contains($rest, 'invalid_report_transition'), 'Triage must enforce an explicit status state machine.');
safety_check(str_contains($rest, 'report_decision_reason_required') && str_contains($rest, 'decision_reason=%s'), 'Material report decisions must require and preserve a reason.');
safety_check(str_contains($rest, 'legal_hold_release_not_authorized') && str_contains($safety, 'sn_network_legal_hold_release_authorized'), 'Legal-hold release must require a separate fail-closed authorization decision.');
safety_check(str_contains($rest, 'appeal_reviewer_conflict') && str_contains($rest, '(int) $row->decision_by === $administrator_id'), 'Appeals must be decided by a different reviewer from the original decision maker.');
safety_check(str_contains($rest, 'report_inventory_viewed'), 'Sensitive report inventory access must be audited.');
...
safety_check(str_contains($safety, "hash('sha256', 'erased-user-report:'") && str_contains($safety, "target_key=%s"), 'Erasure must break user-target correlation keys when no hold applies.');
safety_check(str_contains($policy, 'age_verification_required') && str_contains($policy, 'sn_network_unknown_age_contact_allowed'), 'Unknown age must fail closed for communication contact.');
safety_check(str_contains($auth, "'phone_masked' => \$can_see_phone"), 'Masked phone data must obey the same authorization decision as the full phone.');
