...
r15(str_contains($out,"'contract'=>")&&str_contains($out,"'schema_version'=>")&&str_contains($out,"schema['owner']")&&str_contains($out,"schema['privacy_class']"),'Dispatch envelope must carry governed schema metadata.');
r15(str_contains($out,"sn_network_outbox_delivery_result',false")&&!str_contains($out,"sn_network_outbox_delivery_result',true"),'Outbox acknowledgement must fail closed when no consumer explicitly acknowledges.');
