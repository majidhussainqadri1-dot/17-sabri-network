...
$root=dirname(__DIR__);$top=file_get_contents($root.'/includes/class-sn-top20-communication.php');$fail=[];
$markers=[
 ...
 "hash('sha256', $actor.':'.$conversation_id.':scheduled:'.$client)",
 "status='pending'",
 "status='processing'",
 ...
 "if(!$message||!SN_DB::is_member((int)$message->conversation_id,$actor)||$message->deleted_at)",
 "wp_strip_all_tags((string)$message->body)",
 "body'=>'','attachment_id'=>0,'attachment_source'=>'none'",
...
if($fail){fwrite(STDERR,"Four-plan review 3 failed:\n - ".implode("\n - ",$fail)."\n");exit(1);}echo "Four-plan review 3 security/adversarial: PASS\n";
