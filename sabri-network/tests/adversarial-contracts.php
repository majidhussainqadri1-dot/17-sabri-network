...
// Release reproducibility. Suite filenames are the authoritative explicit inventory,
// and every suite must execute through the warning-fatal canonical PHP test wrapper.
adv_check(
    str_contains($quality, 'static-contracts.php')
    && str_contains($quality, 'adversarial-contracts.php')
    && str_contains($quality, 'php "tools/run-php-test.php" "tests/$test_file"'),
    'Both independent review suites must run through the warning-fatal explicit quality gate.'
);
