...
$check(str_contains($rootreadme,'40 sequential review rounds')&&str_contains($rootreadme,'**18**')&&str_contains($rootreadme,'**22**'),'Root README must publish the founder-requested round count.');
...
$all=$rootreadme."\n".$pluginreadme."\n".$status."\n".$system."\n".$complete;
$check(!preg_match('/(?:100% secure|unhackable|E2EE enabled|production-ready)/i',$all),'Current release documents must not claim unsupported security/production status.');
